...
| \(r_f\) | 3-month Treasury yield | decimal per year |
...
with \(t_{ij} = |T_{ij}|\). Means are the pair's own means, not a global mean:
...
| Sector `weight_pct` | full float, then JSON | \(100\,(E^\top w)_j\) at **max-Sharpe** \(w\) |
