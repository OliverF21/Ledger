...
- Plaid **sandbox** credentials are provided as Cursor Secrets (`PLAID_CLIENT_ID`, `PLAID_SANDBOX_SECRET`), injected as env vars into new VMs. `PLAID_ENV` defaults to `sandbox`. `backend/.env` also holds these values (copied in during setup); note `load_dotenv(override=False)` means real injected env vars win over anything in `.env`, so leftover `.env.example` placeholders are harmless.
- Gotcha: `.env` is read only at process start (uvicorn `--reload` watches `.py` files, not `.env`), so restart the backend after changing Plaid config for it to take effect.
- Headless sandbox link (no Link UI needed), useful for E2E tests: `POST https://sandbox.plaid.com/sandbox/public_token/create` with `client_id`/`secret`/`institution_id` (e.g. `ins_109508`, First Platypus Bank) + `initial_products:["transactions"]` to mint a `public_token`; exchange it via `POST /api/plaid/set_access_token`; then `POST /api/plaid/sync`. The first sync may return 0 transactions (sandbox lag) — wait ~10-15s and re-sync. In the real UI, link any sandbox institution with `user_good` / `pass_good`.
