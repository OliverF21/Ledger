...
export function GlassCard({
  ...
  rise?: boolean
  /** Visible for anything that has to bloom past the card (donut glow). */
  overflow?: 'hidden' | 'visible'
}) {
  return (
    <div
      className={`glass-card ${overflow === 'visible' ? 'overflow-visible' : 'overflow-hidden'} ${rise ? 'ledger-rise' : ''} ${className}`}
      style={style}
    >
      {children}
    </div>
        ...
        return (
          <button
            ...
            className={`text-[10px] font-semibold px-2 py-[2px] transition-colors ${
              active ? 'bg-white text-ledger-accent-on' : 'text-white/50 hover:text-white/80'
            }`}
          >
            {option.label}
          </button>
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <button
        ...
        style={{ background: checked ? '#ffffff' : 'rgba(255,255,255,0.14)' }}
      >
        <span
          className={`w-[14px] h-[14px] rounded-full transition-transform ${checked ? 'bg-ledger-accent-on' : 'bg-white'}`}
          style={{
