...
export default function Overview({ onNavigate, month: selectedMonth }: OverviewProps) {
  ...
  useEffect(() => { setHoverPoint(null) }, [timeRange])
  ...
  return (
    <div className="flex flex-col min-w-0">
      ...
      <section className="relative h-[410px] shrink-0 mt-1.5 overflow-visible pr-14">
        ...
        {/* Spending list + donut. `right-14` matches section `pr-14` — `right-0`
            ignores padding and parks the ring against the page's overflow-x clip,
            which cuts the halo into a hard vertical edge. */}
            ...
            <EmptyState
              ...
              action={
                <button
                  type="button"
                  onClick={() => onNavigate('budgets')}
                  className="text-[11.5px] font-semibold text-white/70 hover:text-white"
                >
                  Set up budgets →
                </button>
              }
