...
- **Desktop:** stored in the OS credential store (macOS Keychain, Windows Credential Manager, Linux Secret Service) via `app/key_store.py`. On first launch after this change, a plaintext `config.json` key is copied into the keychain and, after a verified round-trip, deleted from the file. Unsigned Mac builds store the Keychain item so any app in the logged-in session can read it (otherwise Keychain ACLs follow the code signature and an unsigned update would lose access). That is the same local-user trust as the old `config.json`, but a copied Application Support folder no longer includes the key. If the keychain is unavailable or the user clicks Deny, the app falls back to `config.json` (0600) and logs a warning.
